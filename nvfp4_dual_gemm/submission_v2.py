...
# Optimization: Increased stages to 3 to hide TMA latency
num_acc_stage = 1
num_ab_stage = 3
...
def kernel(
    ...
    * (1.0 / (1.0 + cute.math.exp(-x, fastmath=True))),
    ...
    if warp_idx == 0:
        ...
        # Optimization: Prologue to fill the pipeline stages
        # Issue up to num_ab_stage - 1 loads before starting compute
        cur_produce_idx = 0
        prologue_steps = min(num_ab_stage - 1, k_tile_cnt)
        
        for _ in range(prologue_steps):
            ab_empty = ab_producer.acquire_and_advance()
            
            # TMA load A/B1/B2/SFA/SFB1/SFB2 to shared memory
            cute.copy(
                tma_atom_a,
                tAgA[(None, cur_produce_idx)],
                tAsA[(None, ab_empty.index)],
                tma_bar_ptr=ab_empty.barrier,
            )
            cute.copy(
                tma_atom_b1,
                tBgB1[(None, cur_produce_idx)],
                tBsB1[(None, ab_empty.index)],
                tma_bar_ptr=ab_empty.barrier,
            )
            cute.copy(
                tma_atom_b2,
                tBgB2[(None, cur_produce_idx)],
                tBsB2[(None, ab_empty.index)],
                tma_bar_ptr=ab_empty.barrier,
            )
            cute.copy(
                tma_atom_sfa,
                tAgSFA[(None, cur_produce_idx)],
                tAsSFA[(None, ab_empty.index)],
                tma_bar_ptr=ab_empty.barrier,
            )
            cute.copy(
                tma_atom_sfb1,
                tBgSFB1[(None, cur_produce_idx)],
                tBsSFB1[(None, ab_empty.index)],
                tma_bar_ptr=ab_empty.barrier,
            )
            cute.copy(
                tma_atom_sfb2,
                tBgSFB2[(None, cur_produce_idx)],
                tBsSFB2[(None, ab_empty.index)],
                tma_bar_ptr=ab_empty.barrier,
            )
            cur_produce_idx += 1

        # Execute k_tile loop - Pipeline Steady State
        for k_tile in range(k_tile_cnt):
            # 1. Issue Next Stage (if available)
            if cur_produce_idx < k_tile_cnt:
                ab_empty = ab_producer.acquire_and_advance()
            ...
            # 2. Wait for Current Stage Data
            ab_full = ab_consumer.wait_and_advance()

            # 3. Compute (Data Copy S2T + Math)
