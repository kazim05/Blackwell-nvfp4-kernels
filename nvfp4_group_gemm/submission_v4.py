...
void mbarrier_wait(int mbar_addr, int phase) {
  // Increased timeout for deeper pipeline (4 stages)
...
constexpr int NUM_STAGES = 4;  // Increased from 2 to 4 for better latency hiding
...
void persistent_group_gemm_kernel(
  ...
  for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
    ...
    // Binary search for group (O(log n) instead of O(n))
    int group = 0;
    ...
    if (tid < BLOCK_M) {
      ...
      // Unroll epilogue loops for better instruction scheduling
      #pragma unroll
...
at::Tensor kernel_launch(
  ...
  // Use 3-4 blocks per SM for better occupancy and load balancing
  // Cap at total_tiles to avoid idle blocks
