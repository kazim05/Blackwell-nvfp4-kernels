...
void persistent_group_gemm_kernel(
  ...
  // Cooperatively load ProblemInfo into SMEM
  {
    ...
    const int* src = reinterpret_cast<const int*>(problems);
  ...
  // Optimization 1: Track barrier phases across tiles to avoid reinit
  // TMA barriers: phase flips every NUM_STAGES iterations  
  // MMA barriers: phase flips every NUM_STAGES iterations (offset by NUM_STAGES from TMA)
  // Mainloop barrier: phase flips every tile
  int tma_base_phase = 0;   // Accumulated TMA phase across tiles
  int mma_base_phase = 0;   // Accumulated MMA phase across tiles
  ...
  for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
    ...
    if (warp_id == NUM_WARPS - 2 && elect_sync()) {
      ...
      for (int iter_k = NUM_STAGES; iter_k < num_iters; iter_k++) {
        ...
        // Optimization 1: Use tracked phase instead of reinit
        const int mma_phase = ((iter_k / NUM_STAGES - 1) + mma_base_phase) % 2;
    ...
    // MMA Consumer (warp 5)
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        const int stage_id = iter_k % NUM_STAGES;
        // Optimization 1: Use tracked phase
        const int tma_phase = ((iter_k / NUM_STAGES) + tma_base_phase) % 2;
    ...
    if (tid < BLOCK_M) {
      ...
      for (int m = 0; m < 32 / 16; m++) {
        ...
        for (int n = 0; n < BLOCK_N / 8; n++) {
          ...
          // Vectorized half2 writes with bounds checking
          if (row < M && col < N) {
            half2 out = __float22half2_rn({tmp[0], tmp[1]});
            *reinterpret_cast<half2 *>(C_ptr + row * N + col) = out;
          }
          if (row + 8 < M && col < N) {
            half2 out = __float22half2_rn({tmp[2], tmp[3]});
            *reinterpret_cast<half2 *>(C_ptr + (row + 8) * N + col) = out;
          }
        }
      }
    }

    // Optimization 1: Update phase tracking for next tile
    // TMA barriers flip every NUM_STAGES iters; total flips = ceil(num_iters / NUM_STAGES)
    tma_base_phase = (tma_base_phase + (num_iters + NUM_STAGES - 1) / NUM_STAGES) % 2;
    // MMA barriers: same count but offset  
    mma_base_phase = (mma_base_phase + (num_iters + NUM_STAGES - 1) / NUM_STAGES) % 2;
