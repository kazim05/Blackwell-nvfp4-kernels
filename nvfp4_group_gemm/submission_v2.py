...
// Single-group kernel (optimized path - zero cudaMalloc overhead)
__global__
__launch_bounds__(TB_SIZE)
void kernel_single_gemm(
  const __grid_constant__ CUtensorMap A_tmap,
  const __grid_constant__ CUtensorMap B_tmap,
  const char * const __restrict__ __grid_constant__ SFA_ptr,
  const char * const __restrict__ __grid_constant__ SFB_ptr,
  half * const __restrict__ __grid_constant__ C_ptr,
  ...
  if (warp_id == NUM_WARPS - 2 && elect_sync()) {
    ...
    // C_ptr is already passed as kernel parameter
...
at::Tensor kernel_launch(
  ...
  auto this_kernel = kernel_single_gemm;
  if (smem_size > 48000)
    cudaFuncSetAttribute(this_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);

  // Launch one kernel per group (zero cudaMalloc overhead)
  for (int g = 0; g < num_groups; g++) {
    int M = std::get<0>(problem_sizes[g]);
    int N = std::get<1>(problem_sizes[g]);
    int K = std::get<2>(problem_sizes[g]);

    // Stack allocate tmaps (like nvfp4_dual_gemm_nernst and nvfp4_gemm_nernst)
    CUtensorMap A_tmap, B_tmap;
    init_AB_tmap(&A_tmap, reinterpret_cast<const char*>(A_tensors[g].data_ptr()), M, K, BLOCK_M, BLOCK_K);
    init_AB_tmap(&B_tmap, reinterpret_cast<const char*>(B_tensors[g].data_ptr()), N, K, BLOCK_N, BLOCK_K);

    const char* SFA_ptr = reinterpret_cast<const char*>(SFA_tensors[g].data_ptr());
    const char* SFB_ptr = reinterpret_cast<const char*>(SFB_tensors[g].data_ptr());
    half* C_ptr = reinterpret_cast<half*>(C_tensors[g].data_ptr());

    int grid_m = (M + BLOCK_M - 1) / BLOCK_M;
    int grid_n = (N + BLOCK_N - 1) / BLOCK_N;
    int grid_size = grid_m * grid_n;

    // Launch with grid constants (zero allocation overhead)
    this_kernel<<<grid_size, TB_SIZE, smem_size>>>(
      A_tmap,
      B_tmap,
      SFA_ptr,
      SFB_ptr,
